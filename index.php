...
$relati=ltrim($relati,'/');?>
<h2>Diretório:</h2>
<ul><?php if($dir!=$base){$paret=trim(dirname($relati),'/');
    if($paret!=='')echo '<a href="?dir='.urlencode($paret).'">Voltar</a><br><br>';}
foreach ($file as $files) {
    ...
    else echo '<li><a href="'.htmlspecialchars($RelPastFile).'">'.htmlspecialchars($files).'</a></li><br>';  
}?></ul>